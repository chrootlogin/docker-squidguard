...
ARG SQUIDGUARD_SHA512=6d55bda4c6b597ff52fed95ec336ca2d0b028359332217a552e645a3ac76d6d812926cd837e77cb5e046109c3c6291d52b92f3f7ac987dbf1b6fd17d68f1413d
...
LABEL maintainer="Simon Erhardt <hello@rootlogin.ch>" \
  ...
  org.label-schema.description="Minimal SquidGuard docker image with integrated filter lists based on Alpine Linux." \
...
RUN set -ex \
  ...
  && echo "Verifying both integrity and authenticity of BerkeleyDB ${BERKELEYDB_VERSION}..." \
  && CHECKSUM=$(sha512sum /tmp/db.tgz | awk '{print $1}') \
  && if [ "${CHECKSUM}" != "${BERKELEYDB_SHA512}" ]; then echo "Warning! Checksum does not match!" && exit 1; fi \
  ...
  # Download SquidGuard
  && wget -q http://www.squidguard.org/Downloads/squidGuard-${SQUIDGUARD_VERSION}.tar.gz -O /tmp/squidguard.tgz \
  && echo "Verifying both integrity and authenticity of SquidGuard ${SQUIDGUARD_VERSION}..." \
  && CHECKSUM=$(sha512sum /tmp/squidguard.tgz | awk '{print $1}') \
  && if [ "${CHECKSUM}" != "${SQUIDGUARD_SHA512}" ]; then echo "Warning! Checksum does not match!" && exit 1; fi \
